...
### 事件对象
    ...
    - 这个对象记录了Ushuaia在操作浏览器或者元素时的相关细节信息
    低版本IE中的事件对象不在事件函数中,在window上,印个ebent属性
    ...
    - 事件监听（事件源）
        在事件对象中有一个属性叫target，能够补货到事件触发时的源头是哪个元素，可以节省性能
        注意：
            只有嵌套关系才能获得准确的事件源

    - 在开发的过程中，如果两个元素时嵌套关系，就要小心，事件冒泡（两个事件尽量不要重名）
    - 阻止冒泡：（在事件源上阻止冒泡，就能解决腹肌执行的问题）
...
### 事件
    ...
    onresize
    onfocus 焦点事件
    onblur 失去焦点事件 一个页面只有一个焦点被集火，一开始，页面中有一个焦点，
    当按了tab后，或者直接被集火之后，焦点会从页面中一道下一个或者选中的焦点元素
